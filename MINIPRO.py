...
class AES_SYSTEM():
	...
	def Encrypt(self):
		for self.address in UIWindow.files_list:
			...
			if UIWindow.enc_key_input.text(): # KEY
				if UIWindow.enc_key_label.text() == 'Key (B):': 
					
					if len(UIWindow.enc_key_input.text()) <= 31:
						self.key = pad(UIWindow.enc_key_input.text().encode() ,32) 
			...
			cipher = AES.new(self.key, self.EncSystem()) 
			...
			file_blocks = os.path.getsize(self.address)//UIWindow.USABLE_RAM 
			counter = 0

		
			with open(self.address, 'rb') as file:
				with open(self.address + UIWindow.FILE_EXT, 'wb') as enc_file:
					fb = file.read(UIWindow.USABLE_RAM) 
			...
			if UIWindow.option_Store_key_nonce_in_DB.isChecked():
				self.filehash = self.sha256Hash(self.address + UIWindow.FILE_EXT) 
				self.SaveKeyNonceTag() 	
	...
	def Decrypt(self):
		for self.address in UIWindow.files_list:
			...
			if not (UIWindow.dec_key_input.text() and UIWindow.dec_nonce_input.text()): 
				Logger('error','[AES_D-0]')
				continue

			if UIWindow.dec_key_label.text() == 'Key (H):': 
				try:
				...
				if len(UIWindow.dec_key_input.text()) <= 31:
					self.key = pad(UIWindow.dec_key_input.text().encode(),32) 
			...
			try:
				cipher = AES.new( self.key, self.EncSystem(), nonce=self.nonce ) 
			except ValueError:
			...
			if not dec_verified: 
				if not UIWindow.option_not_decrypted_verified_keep_original_file.isChecked(): 
					if UIWindow.option_Delete_original_file.isChecked():
						self.DeleteOriginalFile()
				if not UIWindow.option_not_verified_keep_key_nonce_DB.isChecked(): 
				...
				if UIWindow.option_Delete_original_file.isChecked(): 
					self.DeleteOriginalFile()
				if UIWindow.option_Delete_key_nonce_after_decryption.isChecked() and self.KeyNonceSearcher(): 
					self.DeleteKeyNonce()
...
class UI(QMainWindow):
	...
	def __init__(self):
		...
		self.setWindowTitle(f'BaseCrack V{str(EX_Version)}') # Window Title and Version Number It Will Provide By master Dev.
		...
		try:
			try:
				...
				if latest_version > EX_Version:
					Logger('info',"There is a newer version! Please update BaseCrack.")
